...
window.jQuery = function (selectorOrArray) {
  ...
  return {
    ...
    parent: function parent() {
      //可能会出现多个array，根据情况进行判断
      var parentArray = [];
      this.each(function (node) {
        if (parentArray.indexOf(node.parentNode) === -1) {
          //去重，防止打出多个相同的
    ...
    children: function children() {
      var chidrenArray = [];
      this.each(function (node) {
        chidrenArray.push.apply(chidrenArray, _toConsumableArray(node.children)); //可能有多个子array
      });
    ...
    siblings: function siblings() {
      ...
      this.each(function (node) {
        ...
        (_siblingArray = siblingArray).push.apply(_siblingArray, _toConsumableArray(Array.from(node.parentNode.children)));

        siblingArray = siblingArray.filter(function (item) {
          return item != node;
        }); //返回符合条件的所有元素
      });
